        ...
        if nodes_data["type"][idx] == "output":
            ...
            for edge in edges_data.itertuples():
                if edge.target == output_id:
                    ...
                    operation = (
                        nodes_data.loc[nodes_data["id"] == connected_id, "data"]
                        .values[0]
                        .get("content", "")
                        .lower()
                    )
                    ...
                    is_constant_list = (
                        []
                    )  # Keep track of whether each dataframe is constant

                    for e in edges_data.itertuples():
                        if e.target == connected_id:
                            input_node_id = e.source
                            if (
                                nodes_data[nodes_data["id"] == input_node_id][
                                    "type"
                                ].values[0]
                                == "input"
                            ):
                                ...
                                if input_df is not None:
                                    ...
                                    # Get the is_constant value for each dataframe
                                ...
                                for d in edges_data.itertuples():
                                    if (
                                        d.source == input_node_id
                                        and nodes_data[nodes_data["id"] == d.target][
                                            "type"
                                        ].values[0]
                                        == "output"
                                    ):
                                        output_as_input_node_id = d.target
                                        if (
                                            output_as_input_node_id
                                            and output_as_input_node_id
                                            in st.session_state["output_database"]
                                        ):
                                            output_df = st.session_state[
                                                "output_database"
                                            ][output_as_input_node_id]
                                            input_dfs.append(output_df)
                                            is_constant_list.append(
                                                "No"
                                            )  # Default "No" for calculated output

                    if input_dfs:
                        max_date = max(
                            df.index.max() for df in input_dfs
                        )  # Get the max date across all dataframes
                        resampled_dfs = [
